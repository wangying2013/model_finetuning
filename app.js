...
// ===== EDIT RENDERERS =====
function editBasic() {
  return `<div class="fg"><label class="fl">任务名称<span class="rq">*</span></label><input class="fi${errCls('name')}" placeholder="请输入精调任务名称，支持英文、数字、中划线" maxlength="50" value="${F.name}" oninput="F.name=this.value;stepErrors=stepErrors.filter(e=>e!=='name')" onblur="renderAllCards()"/>${errTip('name','请输入任务名称')}</div><div class="fg"><label class="fl">任务描述<span class="rq">*</span></label><textarea class="fta${errCls('desc')}" placeholder="请输入任务描述，最多200字" maxlength="200" oninput="F.desc=this.value;stepErrors=stepErrors.filter(e=>e!=='desc')" onblur="renderAllCards()">${F.desc}</textarea>${errTip('desc','请输入任务描述')}</div>`;
}
function editConfig() {
